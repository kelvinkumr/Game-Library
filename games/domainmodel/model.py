...
def make_comment(comment_text: str, user: User, rating: int, game: Game, repository_mode: str, timestamp: datetime = datetime.today()):
    review = Review(user, game, int(rating), comment_text)
    if repository_mode == 'memory':
        user.add_review(review)
        game.add_review(review)
    return review

def make_genre_association(article: Game, genre: Genre):
    if genre.is_applied_to(game):
        raise ModelException(f'Genre {genre.genre_name} already applied to Game "{game.title}"')
